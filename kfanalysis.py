...
class KFanalysis:
    def __init__(self):
        ...
        self.timestamplimit = 1743696000
        ...
        self.timegap_topic_out = 7
        self.timegap_topic_in = 2
    ...
    def get_onetopic_info(self, url, disp=False, index=0):
                ...
                for x in replay_box.contents:
                    if isinstance(x, Tag):
                        if x.name == "img":
                            ...
                            # 引用回复中，引用楼主第0楼与引用其他楼层形制略有区别，但前者也没必要统计
                            un = x.text
                            if "Quote:引用第" in un:
                                ...
                                # tid=1059212&sf=e18的第29楼遇到的特殊情况（这个符号大概是层主自己打的）
                                    un = re.findall("（(.*)）", un)[0]
                                else:
        ...
        for page in range(2, (topic_info["reply_count"] // 20) + 2):
            ...
            if topic_url == "https://bbs.kfpromax.com/read.php?tid=715016&sf=39b":
                if page < 438: continue
            elif topic_url == "https://bbs.kfpromax.com/read.php?tid=1027097&sf=e68":
                if page < 148: continue
            elif topic_url == "https://bbs.kfpromax.com/read.php?tid=798326&sf=457":
                if page < 304: continue
            elif topic_url == "https://bbs.kfpromax.com/read.php?tid=614788&sf=569":
                if page < 167: continue
